...
#ifdef ENABLE_BLER_INSTRUMENTATION
      /* In fixed MCS mode (harq_round_max==1), allow normal HARQ retransmissions (up to 4 rounds) */
      const int effective_msg3_harq_max = (gNB_mac->ul_bler.harq_round_max == 1) ? 4 : gNB_mac->ul_bler.harq_round_max;
...
#ifdef ENABLE_BLER_INSTRUMENTATION
    /* In fixed MCS mode (harq_round_max==1), allow normal HARQ retransmissions (up to 4 rounds) */
    const int effective_ul_harq_max = (nr_mac->ul_bler.harq_round_max == 1) ? 4 : nr_mac->ul_bler.harq_round_max;
